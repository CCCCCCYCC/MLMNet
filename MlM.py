...
class ChangeInformationExtractionModule(nn.Module):
    def __init__(self, in_d, out_d):
        ...
        self.atte = Attention(self.in_d * 3, ca_attention=2)
        self.conv_dr = nn.Sequential(
            nn.Conv2d(self.in_d * 3, self.in_d, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self.in_d),
            nn.ReLU(inplace=True)
        )

        self.vss = VSSBlock(
            hidden_dim=self.in_d,
            drop_path=0.11428571492433548,
            mlp_ratio=2,
            norm_layer=nn.LayerNorm,
            attn_drop_rate=0.0,
            d_state=16,
    ...
    def forward(self, d4, d3, d2):
        ...
        # fusion
        x = torch.cat([d4, d3, d2], dim=1)
        x = self.conv_dr(x)
        x = x.permute(0, 2, 3, 1)
        x_ca = self.vss(x)

        # x_ca = self.ca(x)
        # x_ca = self.atte(x,1,1)
        # x = x + x_ca
        # x_ca = self.vss2(x)
        x = x * x_ca
        # x_ca = self.vss2(x)
...
class GuidedRefinementModule(nn.Module):
    def __init__(self, in_d, out_d):
        ...
        self.conv_d5 = nn.Sequential(
            nn.Conv2d(self.in_d, 768, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(768),
            nn.ReLU(inplace=True)
        )
        self.conv_d4 = nn.Sequential(
            nn.Conv2d(self.in_d, 384, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(384),
            nn.ReLU(inplace=True)
        )
        self.conv_d3 = nn.Sequential(
            nn.Conv2d(self.in_d, 192, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(192),
            nn.ReLU(inplace=True)
        )
        self.conv_d2 = nn.Sequential(
            nn.Conv2d(self.in_d, 96, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(96),
            nn.ReLU(inplace=True)
        )
...
class MLM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.dims = dims
        ca_attentions = [1, 1, 0, 0]
        ca_attentions1 = [1, 1, 0, 0]
        print(ca_attentions)
        print(ca_attentions1)
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer(
                ...
                ca_attention=ca_attentions[i_layer],
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer_up(
                ...
                mlp_ratio=mlp_ratios[i_layer],
                ca_attention=ca_attentions1[i_layer],
                d_state=math.ceil(dims[0] / 6) if d_state is None else d_state,  # 20240109
                drop=drop_rate,
                attn_drop=attn_drop_rate,
        ...
        # self.information = ChangeInformationExtractionModule()
        ...
        self.TF3 = TF(384, 192)
        # self.TF4 = TF(768,384)
        self.inf1 = ChangeInformationExtractionModule(192, 192)
        self.inf2 = ChangeInformationExtractionModule(192, 192)
        self.change = GuidedRefinementModule(192, 1)
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            #    x2 = self.layers_CNN[i](x2)
            #    x2 = x2.permute(0, 2, 3, 1)

            # exit()
            # x_ = x_.permute()
            #    x_ = torch.cat((x1, x2), dim=3)
            # else:
            #    x = x1
            # print(x_.shape)
            # exit()
            ...
            #    x = self.ChannelReducerlist[i](x_)
            #    x = x.permute(0, 2, 3, 1)
            # print(x.shape)
            # exit()
            # x_ = la
    ...
    def forward_FRM(self, skip_list):
        skip_list[0] = skip_list[0].permute(0, 3, 1, 2)
        skip_list[1] = skip_list[1].permute(0, 3, 1, 2)
        skip_list[2] = skip_list[2].permute(0, 3, 1, 2)
        skip_list[2] = self.TF3(skip_list[2])
        skip_list[1] = self.TF2(skip_list[1])
        skip_list[0] = self.TF1(skip_list[0])
        skip_list[2], skip_list[1], skip_list[0] = self.inf1(skip_list[2], skip_list[1], skip_list[0])
        skip_list[2], skip_list[1], skip_list[0] = self.inf2(skip_list[2], skip_list[1], skip_list[0])

        skip_list[2], skip_list[1], skip_list[0] = self.change(skip_list[2], skip_list[1], skip_list[0])
        skip_list[0] = skip_list[0].permute(0, 2, 3, 1)
        skip_list[1] = skip_list[1].permute(0, 2, 3, 1)
        skip_list[2] = skip_list[2].permute(0, 2, 3, 1)
        return skip_list
